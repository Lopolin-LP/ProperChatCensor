    ...
    toPenalty() {
        ...
        */
        ...
        return {
            ...
            reasons: this.matches.reduce((p, v) => ([...p, v.reason]), []),
            text: this.text,
...
class censor {
     ...
     */
    constructor(enginesAndConfig, config) {
        if (config) {
    ...
    run(msg) {
        ...
        const all = allReports.pop()?.addResult(...allReports);
     ...
     */
    static apply(msg, mask, censorChar = '#') {
        if (msg.length !== mask.length)
            throw new Error("Mask does not match Message length");
...
*/
export { censor, censorMatch, engineResult };
